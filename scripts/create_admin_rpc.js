...
// It creates a SECURITY DEFINER function that bypasses RLS

const SUPABASE_ACCESS_TOKEN = 'sbp_6db4905eaab719c79b7475be900a6b0ca8a7d94d';
const PROJECT_REF = 'pbxrtqurwdfoplhbzyhd';

const sql = `
-- Create a function that returns all users with TradingView usernames
-- SECURITY DEFINER means it runs with the privileges of the function creator (superuser)
...
async function applySQL() {
    console.log('Applying SQL via Management API...');
    ...
    if (response.ok) {
        console.log('SUCCESS: get_admin_tv_users() function created!');
    } else {
